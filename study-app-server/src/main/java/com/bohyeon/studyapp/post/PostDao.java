...
public class PostDao {
    ...
    public List<PostResponseDto> findAll() {
        String sql = "SELECT p.id, p.title, p.content, p.created_time, p.category, p.local as location, p.view_count, p.author_id, COUNT(c.id) AS comment_count FROM post p LEFT JOIN comment c ON p.id = c.post_id GROUP BY p.id";
        return namedParameterJdbcTemplate.query(sql, BeanPropertyRowMapper.newInstance(PostResponseDto.class));
    }

    public PostDetailResponse findById(Long id) {
        String sql = "SELECT p.id, p.title, p.content, p.created_time, p.category, p.local as location, p.view_count, p.author_id FROM post p WHERE p.id = :id";
    ...
    public Long addPost(PostRequestDto requestBody, Long userId) {
        ...
        String sql = "INSERT INTO post (title, local , category, created_time, content, author_id, view_count) VALUES (:title, :local, :category, :time,  :content, :authorId, 0)";
        SqlParameterSource parameter = new MapSqlParameterSource("title", requestBody.getTitle())
                .addValue("local", requestBody.getLocation())
    ...
    public void updatePost(Long id, PostRequestDto postRequestDto) {
        String sql = "UPDATE post SET title = :title, category = :category, local = :location, content = :content WHERE post.id = :id";
    ...
    public List<PostResponseDto> findByUserId(Long id) {
        String sql = "SELECT p.id, p.title, p.content, p.created_time, p.category, p.local as location, p.view_count, p.author_id, " +
    ...
    public List<PostResponseDto> findPaginated(int pageNo, int pageSize) {
        ...
        String sql = "SELECT p.id, p.title, p.content, p.created_time, p.category, p.local as location, p.view_count, p.author_id, COUNT(c.id) AS comment_count " +
